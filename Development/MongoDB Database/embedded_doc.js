...
//? 4:we need to return an comments array which must have this two (user = alice & hariom) elements only in it.
//! We need to use $all operator. Here the order doesn’t’ matter.

// db.comments.find({ "comments.user": { $all: ["Alice", "hariom"] } });
// db.comments.find({'comments.user': {$all: ['Alice','hariom','Bob']}})
